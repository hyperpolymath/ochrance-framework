...
D.A. Jewell, The Open University *Purpose:* Week-by-week development
plan with code templates, test specifications, and command references.
____
...
[arabic]
. link:#1-prerequisites[Prerequisites]
. link:#2-project-file-structure[Project File Structure]
. link:#3-phase-1-ochrance-core-weeks-1-12[Phase 1: Ochrance Core (Weeks
1-12)]
. link:++#4-phase-2-echidna--idris2-backend-weeks-13-18++[Phase 2:
ECHIDNA + Idris2 Backend (Weeks 13-18)]
. link:#5-phase-3-neural-synthesis-weeks-19-26[Phase 3: Neural Synthesis
(Weeks 19-26)]
. link:#6-phase-4-production-and-thesis-weeks-27-38[Phase 4: Production
and Thesis (Weeks 27-38)]
. link:#7-test-specifications[Test Specifications]
. link:#8-command-reference[Command Reference]
. link:#9-troubleshooting[Troubleshooting]
...
*Goal:* Implement the filesystem module as the VerifiedSubsystem
reference.
...
|T003 |A2ML Parser |Reject document without header
|`+Left (ParseError ...)+`

|T004 |A2ML Parser |Reject document with null bytes
|`+Left (ParseError ...)+`

|T005 |A2ML Parser |Reject document > 16 MiB
|`+Left (SizeExceeded ...)+`

|T006 |A2ML Parser |Reject nesting depth > 8
|`+Left (NestingExceeded ...)+`

|T007 |A2ML Round-trip |`+parse (serialise doc) == Right doc+` |Property
holds for 10000 random docs

|T008 |Hash |SHA-256 of known input matches expected output |Exact match

|T009 |Hash |SHA3-256 of known input matches expected output |Exact
match
...
[width="100%",cols="21%,28%,15%,36%",options="header",]
|===
|Test ID |Description |Setup |Expected Result
|I001 |Full verification of 1 GB filesystem |nvmet loopback
|VerifiedState with valid Merkle root

|I002 |Detect single bit flip |Inject corruption, verify |`+P+`
diagnostic with block ID

|I003 |Detect metadata corruption |Modify inode, verify |`+Z+`
diagnostic with repair action

|I004 |Repair from CoW snapshot |Corrupt block, repair |Block restored,
`+Q+` diagnostic

|I005 |A2ML attestation round-trip |Verify, serialise, parse, re-verify
|Identical VerifiedState

|I006 |SMART health degradation |Simulate via dm-flakey |`+P+`
diagnostic with wear prediction
|===
...
|A2ML parser |Mutated valid documents |No crashes, graceful error
handling

|NVMe FFI |Random ioctl responses |No memory corruption, proper error
propagation

|Merkle construction |Random hash lists |Correct tree structure, no
panics
|===
...
`+IDRIS2_PACKAGE_PATH+` environment variable includes the build
directory:
...
behaviour). Check that the Ephapax token is consumed exactly once.
Common mistakes:

* Using the token in both branches of an `+if-then-else+` (only one
branch executes, so this is non-linear).
...
*Solution:* This is expected for some neural candidates. The
verification loop will try up to 16 candidates. If all fail:

[arabic]
. Check the proof goal is well-typed by compiling it separately.
. Check that the context (available hypotheses) is complete.
. Fall back to symbolic proof search (slower but more reliable).
. Report the goal to VeriSimDB as a "`hard`" goal for future training.
...
[arabic]
. Increase training data (more proof samples from symbolic search).
. Use data augmentation (proof term transformations).
. Increase beam width (try 32 or 64 instead of 16).
. Check that the tokeniser vocabulary covers Ochrance-specific
identifiers.
. Fine-tune on domain-specific proofs (filesystem properties only).

'''''

=== Appendix A: Weekly Milestone Summary

[width="100%",cols="16%,17%,26%,41%",options="header",]
|===
...
|5 |P1 |VerifiedState + Hash types |`+Ochrance.State+`,
`+Ochrance.Hash+` modules

|6 |P1 |Ephapax + Policy |`+Ochrance.Ephapax+`, `+Ochrance.Policy+`
modules
...
|12 |P1 |Telemetry + integration tests |JSON-lines, OTel, end-to-end
passing
