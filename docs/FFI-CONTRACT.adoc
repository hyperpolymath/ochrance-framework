...
____
*Version:* 1.0.0-draft *Last updated:* 2026-02-19 *Applies to:* Ochrance
L1 (Block Observer) and L2 (Merkle Root) boundary
____

=== 1. Overview

Ochrance uses Idris2 as its primary implementation language for all
verification logic (L2-L4). However, Idris2 cannot directly issue Linux
`+ioctl()+` syscalls or interact with the NVMe driver stack. Therefore,
a thin C shim layer exists at L1 to bridge Idris2 to the hardware.
...
The C shim layer is deliberately minimal and non-intelligent. It
performs exactly three operations:

[arabic]
. Read NVMe SMART data (health/telemetry).
. Read a block from a device.
. Write a block to a device.

The C code does NOT:

* Validate block offsets (Idris2 dependent types enforce
`+offset < device_blocks+`).
...
The `+Fin (totalBlocks dev)+` type ensures that `+offset+` is always
within bounds. There is no runtime bounds check in C because the Idris2
type system has already proven it impossible to construct an
out-of-bounds offset.

==== 2.3 Minimal Attack Surface

The C shim has approximately 200 lines of code total. This is small
enough to be audited line-by-line and formally verified with tools like
Frama-C if desired. The functions have no global state, no heap
allocation, and no complex control flow.
...
`+exit()+`, `+abort()+`, `+longjmp()+`, or any function that transfers
control. Errors are always reported by return value.
...
successfully. A partial read or partial write is an error (returns
`+-EIO+`).
...
* *Mock NVMe device:* A loopback NVMe target (`+nvmet+`) for integration
tests.
* *Error injection:* `+dm-flakey+` device-mapper target to simulate I/O
errors.
* *Valgrind/ASan:* Memory safety verification (no leaks, no
out-of-bounds).
...
* Any valid `+Fin n+` offset produces a successful read on a healthy
device.
...
Rust provides memory safety guarantees that C does not, which is
valuable for a module that introspects process memory. The Idris2 side
remains unchanged – it still calls via `+%foreign+` with C ABI
compatibility.
...
verifier, providing a second layer of safety beyond Idris2’s type
system.
