...
All source files live under `+ochrance-core/Ochrance/Filesystem/+` to
match the Idris2 module hierarchy (sourcedir = "`ochrance-core`"):

[width="100%",cols="41%,59%",options="header",]
|===
|Module |File
|`+Ochrance.Filesystem.Types+`
|`+ochrance-core/Ochrance/Filesystem/Types.idr+`

|`+Ochrance.Filesystem.Merkle+`
|`+ochrance-core/Ochrance/Filesystem/Merkle.idr+`

|`+Ochrance.Filesystem.Verify+`
|`+ochrance-core/Ochrance/Filesystem/Verify.idr+`
...
The filesystem module provides verified integrity checking for
block-based storage. It models storage as a vector of fixed-size blocks
(4096 bytes), with integrity tracked via Merkle trees.
...
these via `+%foreign+` declarations (to be added in
`+Ochrance.FFI.NVMe+`).
