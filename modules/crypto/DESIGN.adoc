...
primitive usage. This is not a crypto library — it verifies that
existing crypto operations are being used correctly:
...
* All actual hash computation is delegated to proven implementations via
FFI
* Policy is declarative (A2ML manifests), not hardcoded
* Quantum-safe algorithm support planned (see
EXHIBIT-B-QUANTUM-SAFE.txt)

=== Open Questions

* Should we verify OpenSSL/BoringSSL configuration, or abstract over
backends?
